...
def menu():
    ...
    button_play = GameSprite(text_play, 250, 110, 200, 100, 0)
    menu = True
    while menu:
        ...
        for e in event.get():
            if e.type == QUIT:
                menu = False
            if btn_play.rect.colidepoint((pos_x,pos_y)) and e.type == MOUSEBUTTONDOWN:
                menu = False
                game()
            
            if btn_exit.rect.colidepoint((pos_x,pos_y)) and e.type == MOUSEBUTTONDOWN:
                menu = False

        pos_x,pos_y = mouse.set_pos()
...
while run:
    ...
    if not finish:
        mw.blit(background, (0, 0))
        text_score = text.render("Рахунок: " + str(score), 1, (255, 255, 255))
